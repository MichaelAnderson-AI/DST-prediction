...
# Create Dataset (Using last 24 time steps to predict the next one)
X_data, y_data = [], []
for i in range(24, len(scaled_data)):
    X_data.append(scaled_data[i-24:i, 0])  # Past 24 values as input
    y_data.append(scaled_data[i, 0])  # Predict next time step

X_data, y_data = np.array(X_data), np.array(y_data)

# Reshape X_data correctly to (num_samples, 24, 1)
X_data = X_data.reshape(X_data.shape[0], 24, 1)
...
torch.save(model.state_dict(), "dst_lstm_model_update.pth")
print("Fine-tuned model saved.")

# Predict Next Value (using the last 24 time steps)
model.eval()
future_input = torch.tensor(scaled_data[-24:].reshape(1, 24, 1), dtype=torch.float32).to(device)
